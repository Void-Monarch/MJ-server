...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new AppError('You are not logged in! Please log in to get access.', 401,res), // throw error if token not found
    );
  }
  ...
  req.user = currentUser; // Puts user Object on request object
  res.locals.user = currentUser; // Puts user Object on response object
  next();
});
